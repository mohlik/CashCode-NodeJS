...
class CashCode {
	constructor(path) {
		this.path = path;
		this.reset = () => port.write(Buffer.from([0x02, 0x03, 0x06, 0x30, 0x41, 0xB3]));
		this.enable = () => port.write(Buffer.from([0x02, 0x03, 0x06, 0x00, 0xC2, 0x82]));
     	this.poll = () => port.write(Buffer.from([0x02, 0x03, 0x06, 0x33, 0xDA, 0x81]));
	 	this.ack = () => port.write(Buffer.from([0x02, 0x03, 0x0C, 0x34, 0x00, 0x00, 0x7C, 0x00, 0x00, 0x00, 0x66, 0xC1]));
		this.port = null;
		this.parser = null;
	...
	init(func) {
		this.port = new SerialPort({ path: this.path, baudRate: 9600 });
	  	this.parser = this.port.pipe(new ByteLengthParser({ length: 7 }));
		this.pollInterval = () => setInterval(this.poll, 500);
		this.port.on('open', () => {
			setTimeout(this.reset, 0);
			setTimeout(this.enable, 5000);
			setTimeout(this.pollInterval, 8000);
		});
